	...
	// Draw time
	{
		...
		if (amount >= 0)
		{
			...
			frameResources[KEY_GUI_HOURGLASS_SAND_TOP] = driver->addRenderTargetTexture(dimension2d<u32>(256, 256));
			{
				...
				vector3df v1(containerSize.X * 0.5f, containerSize.Y * 0.5f, 0.0f);
				vector3df v2(0, 0, rotationValue);

				imageNode.setVertices(containerSize, vertices[0], vertices[1], vertices[2], vertices[3], &v1, &v2);
