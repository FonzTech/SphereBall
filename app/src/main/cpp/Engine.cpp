...
void Engine::loop()
{
	...
	while (device->run() && RoomManager::singleton->isProgramRunning)
	{
        ...
        IRenderTarget* rt = driver->addRenderTarget();
		{
			// Remove all textures and clear array
			for (s32 i = sceneRtts->size() - 1; i >= 0; --i)
			{
			    for (s32 j = (*sceneRtts)[i]->getTexture().size() - 1; j >= 0; --j)
                {
                    driver->removeTexture((*sceneRtts)[i]->getTexture()[j]);
                }
			}
			sceneRtts->clear();

			// Create new textures for render targets
            irr::core::array<ITexture*> textures;
            textures.push_back(driver->addRenderTargetTexture(windowSize, "colorRtt", ECF_A8R8G8B8));
            textures.push_back(driver->addRenderTargetTexture(windowSize, "ppRtt", ECF_A8R8G8B8));
            rt->setTexture(textures, nullptr);
            sceneRtts->push_back(rt);
		}
		// Make G-Buffer current
		driver->setRenderTargetEx(rt, 0);
		...
		*/
		...
		// Work on scene render target
		{
            ...
            irr::core::array<ITexture*> textures = (*sceneRtts)[0]->getTexture();
			screenQuad.getMaterial(0).setTexture(1, textures[0]);
			screenQuad.getMaterial(0).setTexture(2, textures[1]);
...
void Engine::PostProcessing::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{
	...
	dimension2du rttSize = SharedData::singleton->sceneRtts[0]->getTexture()[0]->getOriginalSize();
